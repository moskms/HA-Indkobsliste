...
## 2.0.43
   - Ny "+"-knap ved siden af "Scan bon", til lange bonner der ikke kan fanges læseligt i ét billede: tryk "+", tag et billede, og vælg "Færdig" (send bonnen til Claude nu) eller "Fortsæt" (tag endnu et billede af samme bon) - gentages så mange gange som nødvendigt. Alle billeder sendes samlet til Claude i ét kald, som sammenstiller varelinjerne til ÉN bon. Almindelig "Scan bon" er uændret (ét billede, scanner med det samme). Samtidig hævet Claude-kaldets `max_tokens` fra 2048 til 4096, så en bon med mange varelinjer ikke risikerer at blive afskåret midt i svaret

## 2.0.44
   - Ny envejs-synkronisering til Home Assistants egen "Indkøbsliste" to-do-liste (`todo.indkobsliste`): når en vare tilføjes (manuelt, via stemme, eller via "Fjern + køb igen"), og når en "Over dato"-vare automatisk lægges tilbage på listen ved udløb, sendes den nu også til HA's native liste via `todo.add_item` - så Nabu/Assist kan læse den fulde liste op, uanset hvor varen oprindeligt blev tilføjet. Bruger samme `SUPERVISOR_TOKEN`-mønster som `/diagnostics/ha-position`; fejler stille i loggen hvis HA ikke kan nås, og forhindrer aldrig at varen gemmes i selve appen. Ny hjælpefunktion `sync_item_to_ha()` i `main.py`. Ikke kaldt ved `/restore` (backup-gendannelse), for ikke at spamme HA's liste med gamle varer. Første skridt af den aftalte to-vejs-synkronisering - den anden retning (HA's native liste → appen) er stadig parkeret

## 2.0.45
   - Selve Indkøbsliste-siden havde reelt kun stemmeinput ("🎤 Tilføj vare") - i modsætning til README'ens beskrivelse ("via stemme eller tekst") fandtes der intet tekstfelt der. Tilføjet et simpelt tekstfelt + "Tilføj"-knap under mikrofonknappen, som fallback når stemme ikke er tilgængelig (mikrofon-tilladelse blokeret, browser uden understøttelse, eller usikker HTTP-forbindelse hvor browseren nægter mikrofon-adgang) - browserens "Fejl: not-allowed" på stemmeknappen er præcis denne mikrofon-blokering, ikke en fejl i appen. Tekstfeltet bruger samme `/items`-endpoint og dermed samme HA-synkronisering (`sync_item_to_ha()`, v2.0.44) som stemmeinput
   - Forstørret versionsnummeret i hamburger-menuen (`#menu-version`, fx "v2.0.45") fra 0.85em til 1.2em, så det er mere synligt

## 2.0.46
   - HA-synkroniseringen (v2.0.44) er ændret fra ren tilføjelse til fuld SPEJLING: Home Assistants egen "Indkøbsliste" (`todo.indkobsliste`) bliver nu overskrevet så den PRÆCIS matcher app'ens aktive (ikke-afkrydsede) varer - både tilføjer det der mangler i HA og fjerner det i HA der ikke længere er aktivt i appen. Løser at varer tilføjet direkte i HA/til Nabu (udenom appen), afkrydsede varer, og slettede varer blev siddende i HA's liste for evigt. App'ens liste er den eneste autoritative kilde, indtil den anden retning (HA -> appen) evt. bygges senere
   - Ny hjælpefunktion `mirror_list_to_ha()` i `main.py`, som erstatter den tidligere kun-tilføjende `sync_item_to_ha()`. Bruger `todo.get_items` til at hente HA's nuværende varer, og `todo.remove_item`/`todo.add_item` til at rette forskellen. Kaldes nu også ved afkrydsning og sletning af varer, ikke kun ved tilføjelse. Fortsat ikke kaldt ved `/restore` (backup-gendannelse), og fejler stille i loggen hvis HA ikke kan nås

## 2.0.47
   - Anden retning af den to-vejs-synkronisering: nyt endpoint `GET /webhook/sync-from-ha`, som henter Home Assistants nuværende varer og opretter dem i appen hvis de mangler der - så en vare tilføjet direkte i HA (fx via Nabu/Assist, eller HA's eget to-do-kort) nu også dukker op i selve appen. Fjerner ikke noget fra appen (det er fortsat kun appens egne handlinger, via `mirror_list_to_ha()`, der bestemmer hvad der skal væk fra HA's liste)
   - Kræver en ny HA-automation der kalder det nye endpoint når `todo.indkobsliste` ændres - selve automation-opsætningen i Home Assistant er endnu ikke lavet, kun app-siden af funktionen
